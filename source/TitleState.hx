...
class TitleState extends MusicBeatState {
	...
	override public function create():Void {
		...
		swagShader = new ColorSwap(); curWacky = FlxG.random.getObject(getIntroTextShit());
	...
	function client_onMetaData(e) {
		video.attachNetStream(netStream); video.width = video.videoWidth; video.height = video.videoHeight;
	}
	...
	function overlay_onMouseDown(e) {
		netStream.soundTransform.volume = 0.2; netStream.soundTransform.pan = -1;
	...
	function startIntro() {
		if (!initialized) {
			var diamond:FlxGraphic = FlxGraphic.fromClass(GraphicTransTileDiamond); diamond.persist = true;

			FlxTransitionableState.defaultTransIn = transIn = new TransitionData(FADE, FlxColor.BLACK, 1, new FlxPoint(0, -1), {asset: diamond, width: 32, height: 32},
		...
		logoBl = new FlxSprite(-150, -100); logoBl.frames = Paths.getSparrowAtlas('logoBumpin');
		logoBl.animation.addByPrefix('bump', 'logo bumpin', 24);
		logoBl.animation.play('bump'); logoBl.updateHitbox();
		logoBl.shader = swagShader.shader; logoBl.antialiasing = true;
		add(logoBl);
		...
		gfDance.shader = swagShader.shader; gfDance.antialiasing = true;
		add(gfDance);

		titleText = new FlxSprite(100, FlxG.height * 0.8); titleText.frames = Paths.getSparrowAtlas('titleEnter');
		titleText.animation.addByPrefix('idle', "Press Enter to Begin", 24); titleText.animation.addByPrefix('press', "ENTER PRESSED", 24);
		titleText.animation.play('idle'); titleText.updateHitbox();
		...
		ngSpr.visible = false;
		ngSpr.setGraphicSize(Std.int(ngSpr.width * 0.8)); ngSpr.updateHitbox();
		ngSpr.screenCenter(X); ngSpr.antialiasing = true;
		add(ngSpr);
	...
	override function update(elapsed:Float) {
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			titleText.animation.play('press'); FlxG.camera.flash(FlxColor.WHITE, 1); FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
			transitioning = true; FlxG.switchState(new MainMenuState());
		}
	...
	function createCoolText(textArray:Array<String>) {
		for (i in 0...textArray.length) {
			var text:Alphabet = new Alphabet(0, 0, textArray[i], true, false); text.screenCenter(X);
			text.y += (i * 60) + 200;
			credGroup.add(text);
		}
	}
	...
	function addMoreText(_text:String) {
		var text:Alphabet = new Alphabet(0, 0, _text, true, false); text.screenCenter(X);
	...
	function skipIntro():Void {
		if (!skippedIntro) {
			...
			skippedIntro = true;
			remove(ngSpr, true); remove(credGroup, true);
		}
	}
}
