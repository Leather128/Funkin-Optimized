...
class MainMenuState extends MusicBeatState {
	...
	override function create() {
		...
		var bg:FlxSprite = new FlxSprite(null, null, Paths.image('menuBG')); bg.scrollFactor.set(0, 0.17);
		bg.setGraphicSize(Std.int(bg.width * 1.2));
		bg.updateHitbox(); bg.screenCenter(); bg.antialiasing = true;
		add(bg);

		camFollow = new FlxObject(0, 0, 1, 1); add(camFollow);

		magenta = new FlxSprite(null, null, Paths.image('menuDesat')); magenta.scrollFactor.set(bg.scrollFactor.x, bg.scrollFactor.y);
		magenta.setGraphicSize(Std.int(bg.width)); magenta.updateHitbox();
		magenta.setPosition(bg.x, bg.y);
		magenta.visible = false; magenta.antialiasing = true; magenta.color = 0xFFFD719B;
		if (PreferencesMenu.preferences.get('flashing-menu')) add(magenta);

		menuItems = new MainMenuList(); menuItems.enabled = false; add(menuItems);
		...
		if (VideoState.seenVideo) menuItems.createItem(null, null, "kickstarter", selectDonate, true);
		else menuItems.createItem(null, null, "donate", selectDonate, true);
		...
		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, "v" + Application.current.meta.get('version'), 12); versionShit.scrollFactor.set();
	...
	override function finishTransIn() {
		super.finishTransIn(); menuItems.enabled = true;
	}
	...
	{
		menuItems.enabled = false; menuItems.forEach(function(item:MainMenuItem)
		{
...
class MainMenuItem extends AtlasMenuItem {
	public function new(?x:Float = 0, ?y:Float = 0, name:String, atlas:FlxAtlasFrames, callback:Dynamic) {
		super(x, y, name, atlas, callback); this.scrollFactor.set();
	}

	override public function changeAnim(anim:String) {
		super.changeAnim(anim); origin.set(frameWidth * 0.5, frameHeight * 0.5); offset.copyFrom(origin);
	}
}
...
class MainMenuList extends MenuTypedList<MainMenuItem> {
	...
	public function new() {
		atlas = Paths.getSparrowAtlas('main_menu'); super(Vertical);
	}

	public function createItem(?x:Float = 0, ?y:Float = 0, name:String, callback:Dynamic = null, fireInstantly:Bool = false) {
		var item:MainMenuItem = new MainMenuItem(x, y, name, atlas, callback); item.fireInstantly = fireInstantly; item.ID = length;
